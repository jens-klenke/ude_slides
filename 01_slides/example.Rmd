...
.font70[
With Rmarkdown/xaringan `R` code can directly implemented. There are two ways to implement R Code. If we want to use r code *inline* we put the command into .ilr[r ]. For example if we want to write some continuous text and with the number of variables of the `mtcars` data set has, we just write .ilr[r dim(mtcars)[2]] (`mtcars` has `r dim(mtcars)[2]` variables).  
...
.pull-down[
More information how to use and format `R` output in a `R Markdown` can be found on the [RStudio](https://rmarkdown.rstudio.com/lesson-1.html) side. 
]
